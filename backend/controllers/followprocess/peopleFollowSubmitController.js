...
const getPeopleFollowSubmit = async (req, res) => {
    ...
    try {
        ...
        const submitted = await Peoplefollowing.aggregate([
...
const addPeopleFollowSubmit = async (req, res) => {
    ...
    try {
        ...
        const foundEdit = await Peoplefollowing.findOne({_userId: loggedUserId, "submittedFollowRequests._submittedToUser": otherUserId})

        if(foundEdit){

            const foundRequests = await Peoplefollowing.updateOne(
            ...
            const foundNew = await Peoplefollowing.updateOne({_userId: loggedUserId},{$push:{submittedFollowRequests: {_submittedToUser: otherUserId}}})
...
const editPeopleFollowSubmit = async (req, res) => {
    ...
    try {
        ...
        const found = await Peoplefollowing.updateOne({ 
...
module.exports = { getPeopleFollowSubmit, addPeopleFollowSubmit, editPeopleFollowSubmit }