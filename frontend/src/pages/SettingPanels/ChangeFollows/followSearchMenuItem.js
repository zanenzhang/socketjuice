...
export default function FollowSearchMenuItem({loggedUserId, followingUsername, followingUserId, 
    followingUserProfilePicURL, followingUsers, submittedUsers, changedData, setChangedData, 
    blockedUsers, privacySetting, loggedUserOrStore, roles}){
    ...
    useEffect( ()=> {
        ...
        async function getData(){
            ...
            let isFollowingProfile = null
            let isRequestedProfile = null
            let isBlockedProfile = null
            ...
            if(followingUsers?.length > 0){
                isFollowingProfile = followingUsers.some(e => e._id === followingUserId)
            ...
            if(submittedUsers?.length > 0){
                isRequestedProfile = submittedUsers.some(e => e._id === followingUserId)
            ...
            if(blockedUsers){
                isBlockedProfile = blockedUsers.some(e => e._userId === followingUserId)
                if(isBlockedProfile){
                    setIsBlocked(true)
                } else{
                    setIsBlocked(false)
        ...
        if(followingUsers && blockedUsers && submittedUsers && roles){
            getData();
        }

    }, [followingUsers, blockedUsers, submittedUsers, roles])
