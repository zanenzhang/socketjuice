...
const SalesDisplay = (loggedUserId, loggedUsername, profilePicURL, roles) => {
            ...
            >
              <Box sx={{ ...boxStyleOrder, width: 350 }}>
                ...
                <div className='flex flex-col gap-y-2'>
                    ...
                    {(!completedOrder && preorder && preorderDeadline) && <div className='text-sm pt-6'>
                        <p><b>Group Buy</b> </p>
                    ...
                    <div className='text-sm pt-2'>
                        <p><b>Order Date:</b> {format(new Date(orderDate), "ccc',' LLLL d',' yyyy h':'mm bb")}</p>
                    </div>
    ...
    async function getOrderData(){
        ...
        const orderData = await getUserSales(auth.userId, dateStart, dateEnd, auth.accessToken, auth.userId)
    ...
    return(
        ...
        <div className='text-2xl text-center pt-2'>
            ...
            <div className='flex flex-col md:flex-row w-full border-y-2 md:border-0'>
            ...
            {orders?.length > 0 ?
                ...
                <div className='flex flex-col md:flex-row hover:bg-gray-200 w-full border-b md:border-0'
                ...
                <div className='flex flex-row items-center justify-evenly md:border-b w-full py-4'
                    onClick={(event)=>{openModalSummary(event, order._id, order.productname, order.recipient, order.address_line1, order.address_line2, order.address_region, 
                        order.address_country, order.phoneNumber, order.zipCode, order._postId, order.paidSubtotalPrice, order.paidShipping, order.paidTotalPrice, 
                        order.currencySymbol, order.paidCurrency, order.createdAt, !order.active, order.refundCompleted, order.creditsCompleted, order.shippingDateEarly, order.shippingDateLate, 
                        order.orderStatus, order.preorderSwitch, order.preorderDeadline,  order.selectedSize, order.selectedColor, order.selectedModel, order.previewMediaURL)}}>
                    
                <div className='w-[80px] md:w-[150px] whitespace-pre-line'> <p className='text-center text-xs sm:text-sm'>{order.shippingDateEarly ? `${format(new Date(order.shippingDateEarly), "ccc',' LLLL d',' yyyy")} to ${format(new Date(order.shippingDateLate), "ccc',' LLLL d',' yyyy")}` : 'Not Available'}</p></div>
        ...
        <OrderSummaryModal orderId={orderId} name={name} productname={productname}
